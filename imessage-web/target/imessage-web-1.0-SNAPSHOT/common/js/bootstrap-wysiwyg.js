(function($){
	...
	$.fn.wysiwyg = function(userOptions) {
		var editor = this, selectedRange, options, toolbarBtnSelector, updateToolbar = function() {
			...
			if (text) {
				// 鎻掑叆杩欎釜鏂囨。纰庣墖
				new_a.appendChild(r.cloneContents());
			} else {
				new_a.text = value;
			}
			r.deleteContents();
			r.insertNode(new_a);
		}, execCommand = function(commandWithArgs, valueArg, elem) {
			var commandArr = commandWithArgs.split(' '), command = commandArr
					.shift(), args = commandArr.join(' ') + (valueArg || '');
			if (command === "createLink" && elem
					&& $(elem).prev('.select_div').find('select').val() * 1 === 1) {
				// 鍙栨秷鏍囪
				saveSelection();
				markSelection($(elem), false);
				restoreSelection();
				createLinks(args);
			} else {
			...
			if (getCurrentRange()
					&& $(getCurrentRange().commonAncestorContainer).closest(
							'.editor').length === 0) {
				return;
			}
			...
			$.each(files,
					function(idx, fileInfo) {
						if (/^image\//.test(fileInfo.type)) {
							$.when(readFileIntoDataUrl(fileInfo)).done(
									function(dataUrl) {
			...
			toolbar.find('input[type=text][data-' + options.commandRole + ']').on(
					'webkitspeechchange change',
					function() {
						var newValue = this.value;
						/*
						 * ugly but prevents fake double-calls due to selection
						 * restoration
						 */
						this.value = '';
						restoreSelection();
			...
			editor.on('dragenter dragover', false).on(
					'drop',
					function(e) {
		...
		toolbarBtnSelector = 'a[data-' + options.commandRole + '],button[data-'
		...
		$(window)
				.bind(
						'touchend',
						function(e) {
							var isInside = (editor.is(e.target) || editor
									.has(e.target).length > 0), currentRange = getCurrentRange(), clear = currentRange
